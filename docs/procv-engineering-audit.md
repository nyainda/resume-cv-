...
| S3 | **Confidence-Tagged Fields** | ✅ **SHIPPED** | `TaggedValue<T>` + `ConfidenceLevel` in `types.ts`. `LockedValues` extended with `certifications`, `awardsAndHonors`, `leadershipSignals`. `lockRealNumbers` extracts all three from `customSections` + responsibilities. `buildPromptAnchorBlock` now emits a `=== CREDENTIAL & CLAIM INTEGRITY [S3] ===` block with 3-tier anchor (user_supplied / system_extracted / FORBIDDEN). `ruleUngroundedCertifications` (warn) added to `cvValidationEngine.ts` with 25 credential patterns (AWS, PMP, CISSP, ACCA, CFA, Six Sigma, etc.); wired into `geminiService.ts` via `opts.certifications`. | — |
| S6 | **Profession Ontology** | ✅ **SHIPPED** | `frontend/data/fieldOntology.ts` — 30-node tree (6 root groups + 24 leaves covering all CVField slugs). `frontend/services/fieldOntologyResolver.ts` — `resolveFieldAncestors()`, `getLeafChildren()`, `buildFlatOntology()`, `getFieldLabel()`, `isDescendantOf()`. `UserProfile.preferredField` added to `types.ts`. `detectField()` in `cvPromptHelpers.ts` short-circuits to `preferredField` when set (with validation guard). "Industry / Field" select dropdown added to ProfileForm Personal Info tab — grouped by ontology category, defaults to "Auto-detect". | — |
