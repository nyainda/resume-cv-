...
const generatePdfForTemplate = (
    ...
    // Shared link / interactive-text colour that follows the accent
    const linkColor = hexToRgb(accentHex ?? '', [37, 99, 235]);

    // Common function to draw a section title
    const drawSectionTitle = (title: string, options: any = {}) => {
        const defaultSecColor = hexToRgb(accentHex ?? '', [30, 41, 59]);
        const { yPos, xPos = margin, width = contentWidth, font = selectedFont, size = 10, style = 'bold', color = defaultSecColor, align = 'left', lineColor = [203, 213, 224], lineWidth = 0.5, yMarginBottom = 15, yMarginTop = 15 } = options;
    ...
    const twoColumn = (sidebarColor: [number, number, number], mainColor: [number, number, number]) => {
        ...
        if (cvData.projects && cvData.projects.length > 0) {
            ...
            cvData.projects.forEach(proj => {
                ...
                if (proj.link) {
                    const linkWidth = doc.getTextWidth(proj.name);
                    h.writeText('[Link]', mainContentX + linkWidth + 5, mainY, { font: selectedFont, size: 9, color: linkColor, link: proj.link });
                }
                mainY += 14;
    ...
    const minimalist = () => {
        ...
        if (cvData.projects && cvData.projects.length > 0) {
            ...
            cvData.projects.forEach(proj => {
                ...
                if (proj.link) {
                    const linkWidth = doc.getTextWidth(proj.name);
                    h.writeText('[Link]', margin + dateWidth + 10 + linkWidth + 5, startProjY, { font: selectedFont, size: 8, color: linkColor, link: proj.link });
                }
    ...
    const softwareEngineerPdf = () => {
        ...
        if (linksInfo.length > 0) {
            ...
            h.writeText(linksString, pageWidth / 2, h.getY(), { font: mainFont, size: 9, color: linkColor, align: 'center' });
        }
        h.setY(h.getY() + 25);
        ...
        if (cvData.projects && cvData.projects.length > 0) {
            ...
            cvData.projects.forEach(proj => {
                ...
                if (proj.link) {
                    h.writeText('[Link]', localMargin + nameWidth + 5, startProjY, { font: mainFont, size: 9, color: linkColor, link: proj.link });
                }
    ...
    const classic = () => {
        ...
        if (links) {
            h.writeText(links, pageWidth / 2, h.getY(), { font: selectedFont, size: 9, align: 'center', color: linkColor });
            h.setY(h.getY() + 20);
        } else {
            h.setY(h.getY() + 8);
        }
    ...
    const siliconValleyPdf = () => {
        const blue = hexToRgb(accentHex ?? '', [37, 99, 235]);
