...
// Safely access the API key to avoid a ReferenceError in browser environments.
// The SDK will handle the case where the key is missing when an API call is made,
...
export const generateCV = async (profile: UserProfile, jobDescription: string, enableEnhancements: boolean): Promise<CVData> => {
  ...
  if (enableEnhancements) {
    ...
    cvDataSchema = {
      ...
      properties: {
        ...
        experience: {
            type: Type.ARRAY,
            description: "Work experience, including all of the user's provided experience plus one or two new ideal experiences. Rephrase responsibilities to highlight relevant achievements using action verbs.",
            items: baseExperienceItems
        },
        projects: {
            type: Type.ARRAY,
            description: "Generate 1-2 fictional but realistic project examples that would be impressive for this role. Include a brief description and plausible-looking GitHub or website links for each.",
            items: baseProjectItems
        }
      },
    ...
    projectsInstruction = `6.  **Projects**: Use ONLY the projects provided by the user. If the user provided projects, rephrase their descriptions to better align with the job. If the user did not provide any projects, this section can be omitted from the output. Do not invent new projects.`;
     ...
     cvDataSchema = {
      ...
      properties: {
        ...
        experience: {
            type: Type.ARRAY,
            description: "The user's work experience, with responsibilities rephrased to highlight achievements relevant to the job description.",
            items: baseExperienceItems
        },
        projects: {
            type: Type.ARRAY,
            description: "The user's projects, with descriptions rephrased to be more relevant to the job. If the user provided no projects, this should be an empty array.",
            items: baseProjectItems
        }
      },
      required: ["summary", "experience", "skills", "education", "languages"]
  ...
  const prompt = `
    ...
    9.  Return ONLY the JSON object adhering to the provided schema. Do not include any markdown formatting like \`\`\`json.
  `;
  ...
  try {
    ...
    if (error instanceof Error && error.message.includes('JSON')) {
         throw new Error("Failed to generate CV. The AI model returned an invalid format. Please try again.");
    }
    throw new Error("Failed to generate CV. The AI model may be overloaded. Please try again later.");
  }
};
...
export const generateProfile = async (rawText: string): Promise<UserProfile> => {
    ...
    try {
        ...
        throw new Error("Failed to generate profile from text. Please check the input and try again.");
    }
};
...
export const extractTextFromImage = async (base64Image: string, mimeType: string): Promise<string> => {
    ...
    try {
        ...
        throw new Error("Failed to read text from the image. Please try a clearer image or paste the text manually.");
    }
};
...
export const extractProfileTextFromFile = async (base64Data: string, mimeType: string): Promise<string> => {
    ...
    try {
        ...
        if (error instanceof Error && error.message.includes('unsupported')) {
             throw new Error("Failed to read text from the file. The file type might not be supported by the AI. Please try a common image format (PNG, JPG) or a standard PDF.");
        }
        throw new Error("Failed to read text from the file. Please try a clearer file or paste the text manually.");
    }
};
...
export const analyzeJobDescriptionForKeywords = async (jobDescription: string): Promise<JobAnalysisResult> => {
    ...
    try {
        ...
        throw new Error("Failed to analyze job description. The AI model might be busy.");
    }
};