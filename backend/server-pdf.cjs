...
// ─── CV Pipeline Rules endpoint (dev mirror of the CF Worker rules endpoint) ───
//
// In production these strings live exclusively inside the compiled Cloudflare
// Worker — never in the client bundle. This local endpoint mirrors that
// behaviour for development so the client rulesService can fetch them without
// a live CF deployment.

const CV_HUMANIZATION_RULES = `
...
const CV_HUMANIZATION_CHECKLIST = `
...
const CV_SYSTEM_VALIDATOR = 'You are a strict CV quality validator. Return only valid JSON.';
const CV_SYSTEM_AUDIT = 'You are a strict CV editor. Fix only the listed problems. Return only valid JSON with keys: summary and experience.';

app.get('/api/cv/rules', (_req, res) => {
    res.setHeader('Cache-Control', 'public, max-age=3600');
    res.json({
        version: '2026-05',
        systemProfessional:    CV_SYSTEM_PROFESSIONAL,
        humanizationRules:     CV_HUMANIZATION_RULES,
        humanizationChecklist: CV_HUMANIZATION_CHECKLIST,
        systemHumanizer:       CV_SYSTEM_HUMANIZER,
        systemParser:          CV_SYSTEM_PARSER,
        systemValidator:       CV_SYSTEM_VALIDATOR,
        systemAudit:           CV_SYSTEM_AUDIT,
    });
});
...
app.use('/cf-engine', async (req, res) => {
    const workerPath = req.url || '/';
