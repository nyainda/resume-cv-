        ...
        <div id="cv-preview-swe-elite" className="bg-white flex font-sans text-sm" style={{ minHeight: '297mm', fontFamily: "'Inter', system-ui, sans-serif" }}>
            ...
            <aside className="w-[220px] flex-shrink-0 bg-[#0f172a] text-white flex flex-col py-8 px-6 space-y-7" style={{ minHeight: '297mm' }}>
                ...
                <div className="text-center">
                    <div className="w-16 h-16 rounded-2xl bg-emerald-500 flex items-center justify-center text-2xl font-black text-white mx-auto mb-3 ring-4 ring-emerald-500/30">
                        {initials}
                    </div>
                    <h1 className="text-lg font-black leading-tight text-white">{personalInfo.name}</h1>
                    {cvData.experience[0] && (
                        <p className="text-emerald-400 text-xs font-semibold mt-1">{cvData.experience[0].jobTitle}</p>
                    )}
                </div>

                {/* Contact */}
                <div className="space-y-2">
                    <h2 className="text-[9px] font-black uppercase tracking-[0.15em] text-emerald-400 border-b border-emerald-400/30 pb-1.5">Contact</h2>
                    {[
                ...
                {(personalInfo.github || personalInfo.linkedin || personalInfo.website) && (
                    <div className="space-y-2">
                        <h2 className="text-[9px] font-black uppercase tracking-[0.15em] text-emerald-400 border-b border-emerald-400/30 pb-1.5">Links</h2>
                        {personalInfo.github && (
                            <div className="flex items-center gap-1.5">
                                <svg className="w-2.5 h-2.5 text-emerald-400 flex-shrink-0" viewBox="0 0 24 24" fill="currentColor"><path d="M12 0C5.374 0 0 5.373 0 12c0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576C20.566 21.797 24 17.3 24 12c0-6.627-5.373-12-12-12z" /></svg>
                                <span className="text-[10px] text-slate-300 truncate leading-tight">{personalInfo.github.replace('https://github.com/', '')}</span>
                            </div>
                        )}
                        {personalInfo.linkedin && (
                            <div className="flex items-center gap-1.5">
                                <svg className="w-2.5 h-2.5 text-emerald-400 flex-shrink-0" viewBox="0 0 24 24" fill="currentColor"><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z" /></svg>
                ...
                {skillCategories.length > 0 && (
                    <div className="space-y-4">
                        <h2 className="text-[9px] font-black uppercase tracking-[0.15em] text-emerald-400 border-b border-emerald-400/30 pb-1.5">Tech Stack</h2>
                            ...
                            <div key={cat}>
                                ...
                                <div className="flex flex-wrap gap-1">
                                    {skills.slice(0, 8).map(skill => (
                                        <span key={skill} className="text-[8px] px-1.5 py-0.5 rounded bg-emerald-500/10 border border-emerald-500/30 text-emerald-300 font-mono font-semibold leading-tight">
                                            {skill}
                                        </span>
                                    ))}
                                </div>
                ...
                {cvData.education.length > 0 && (
                    <div className="space-y-2">
                        <h2 className="text-[9px] font-black uppercase tracking-[0.15em] text-emerald-400 border-b border-emerald-400/30 pb-1.5">Education</h2>
                        {cvData.education.slice(0, 2).map((edu, i) => (
                            <div key={i}>
                                ...
                                <p className="text-[9px] text-emerald-400/80">{edu.year}</p>
                            </div>
                        ))}
                    </div>
                )}
                ...
                {cvData.languages && cvData.languages.length > 0 && (
                    <div className="space-y-2">
                        <h2 className="text-[9px] font-black uppercase tracking-[0.15em] text-emerald-400 border-b border-emerald-400/30 pb-1.5">Languages</h2>
                        {cvData.languages.map((lang, i) => (
                            <div key={i} className="flex justify-between items-center">
                                <span className="text-[10px] text-slate-300">{lang.name}</span>
                                <span className="text-[9px] text-emerald-400/80 font-semibold">{lang.proficiency}</span>
                            </div>
                        ))}
                    </div>
                )}
                    ...
                    <section>
                        ...
                        <div className="space-y-5">
                            {cvData.experience.map((job, i) => (
                                <div key={i} className="relative pl-4 border-l-2 border-emerald-100 hover:border-emerald-400 transition-colors">
                                    ...
                                    <ul className="mt-2 space-y-1">
                                        {job.responsibilities.map((resp, j) => (
                                            <li key={j} className="flex items-start gap-2 text-xs text-slate-600 leading-relaxed">
                                                <span className="text-emerald-500 mt-0.5 font-black flex-shrink-0">▸</span>
                                    ...
                                    <div className="flex items-start justify-between gap-2">
                                        ...
                                        {proj.link && (
                                            <a href={proj.link} target="_blank" rel="noopener noreferrer"
                                                className="text-[9px] text-emerald-600 font-bold hover:underline flex-shrink-0 flex items-center gap-0.5">
