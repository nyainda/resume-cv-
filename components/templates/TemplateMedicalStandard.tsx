...
const TemplateMedicalStandard: React.FC<TemplateProps> = ({ cvData, personalInfo, isEditing, onDataChange, jobDescriptionForATS }) => {
    ...
    const SectionHeading = ({ children }: { children: React.ReactNode }) => (
        <div className="flex items-center gap-3 mb-6 mt-12 first:mt-0">
            <div className="flex items-center justify-center w-8 h-8 rounded bg-sky-600 text-white font-black text-xl">+</div>
    ...
    return (
        <div id="cv-preview-medical-standard" className="bg-[#f0f4f8] p-12 sm:p-20 text-slate-800 shadow-xl border-t-[12px] border-sky-600 font-sans leading-relaxed" style={{ fontFamily: "'Inter', sans-serif" }}>
            <header className="bg-white p-12 rounded-3xl shadow-sm border border-slate-100 flex justify-between items-center mb-12">
                <div>
                    <h1 className="text-4xl font-black tracking-tight text-slate-900 mb-2 uppercase">
                        {personalInfo.name}
                        <span className="text-sky-600 ml-2">, MD/RN</span>
                    </h1>
                ...
                <div className="hidden lg:block">
                    <div className="w-16 h-16 rounded-2xl border-2 border-sky-100 flex items-center justify-center bg-sky-50/50">
            ...
            <main className="grid grid-cols-12 gap-12">
                <div className="col-span-12 lg:col-span-8">
                    <section className="bg-white p-8 rounded-3xl border border-slate-100 shadow-sm mb-12">
                        <h2 className="text-xs font-black uppercase tracking-widest text-sky-600 mb-3">Clinical Summary</h2>
                                ...
                                <div key={index} className="relative group bg-white p-8 rounded-3xl border border-slate-100 shadow-sm">
                                    ...
                                    <div className="flex flex-col sm:flex-row justify-between items-start mb-6 gap-4">
                                        <div>
                                            ...
                                            <p className="text-sm font-bold text-sky-600 uppercase tracking-wider mt-2">{job.company}</p>
                                        </div>
                                    ...
                                    <ul className="grid grid-cols-1 gap-4">
                                        {job.responsibilities.map((resp, i) => (
                                            <li key={i} className="flex gap-4 items-start text-sm text-slate-600">
                                                <div className="w-1.5 h-1.5 rounded-full bg-sky-200 mt-2 shrink-0" />
                ...
                <aside className="col-span-12 lg:col-span-4 space-y-12">
                    <section className="bg-sky-600 text-white p-8 rounded-3xl shadow-xl">
                        <h2 className="text-xs font-black uppercase tracking-widest text-sky-200 mb-6 flex items-center gap-2">
                            <span>Clinical Skills</span>
                            <div className="h-px bg-sky-400 flex-1 opacity-30" />
                        </h2>
                    ...
                    <section className="bg-white p-8 rounded-3xl border border-slate-100 shadow-sm">
                        ...
                        <div className="space-y-8">
                            {cvData.education.map((edu, idx) => (
                                <div key={idx} className="relative pl-6 before:absolute before:left-0 before:top-2 before:bottom-0 before:w-0.5 before:bg-sky-100">
                                    <h3 className="text-base font-black text-slate-800" {...editableProps(['education', idx, 'degree'])}>{edu.degree}</h3>
                                    <p className="text-xs font-bold text-sky-600 uppercase tracking-wider mt-1">{edu.school}</p>
                            ...
                            <div className="space-y-4">
                                {cvData.languages.map((lang, idx) => (
                                    <div key={idx} className="flex justify-between items-center text-sm">
                                        ...
                                        <span className="text-[10px] font-black text-sky-600 uppercase tracking-widest bg-sky-50 px-2 py-0.5 rounded-full">{lang.proficiency}</span>
                                    </div>
                                ))}
                            </div>
                        </section>
