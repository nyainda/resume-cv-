...
const JobPipelineModal: React.FC<JobPipelineModalProps> = ({
    ...
    const handleGenerateCV = useCallback(async () => {
        ...
        setCvSaved(false);
        try {
            const cv = await generateCV(userProfile, job.jobDescription || job.snippet, selectedMode, 'job', 'standard');
            setGeneratedCV(cv);
        } catch (e) {
            setCvError(e instanceof Error ? e.message : 'CV generation failed.');
        } finally {
