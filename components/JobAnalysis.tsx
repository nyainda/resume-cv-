        ...
        <div className="mt-6 p-4 border rounded-lg bg-zinc-50 dark:bg-neutral-700/30 border-zinc-200 dark:border-neutral-700/50">
            ...
            {analysis && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <h4 className="font-semibold text-sm mb-2 text-zinc-800 dark:text-zinc-200">Top Keywords</h4>
                        <div className="flex flex-wrap gap-2">
                            {analysis.keywords.map(kw => {
                                const isMatched = matchedKeywords.has(kw);
                                return (
                                    <span key={kw} className={`flex items-center text-xs font-medium px-2.5 py-1 rounded-full transition-all ${isMatched ? 'bg-green-100 text-green-800 dark:bg-green-900/50 dark:text-green-300' : 'bg-zinc-200 text-zinc-700 dark:bg-neutral-600 dark:text-zinc-200'}`}>
                     ...
                     <div>
                        <h4 className="font-semibold text-sm mb-2 text-zinc-800 dark:text-zinc-200">Essential Skills</h4>
                         <div className="flex flex-wrap gap-2">
                            {analysis.skills.map(skill => {
                                const isMatched = matchedSkills.has(skill);
                                return (
                                    <span key={skill} className={`flex items-center text-xs font-medium px-2.5 py-1 rounded-full transition-all ${isMatched ? 'bg-green-100 text-green-800 dark:bg-green-900/50 dark:text-green-300' : 'bg-zinc-200 text-zinc-700 dark:bg-neutral-600 dark:text-zinc-200'}`}>
                                      {isMatched && <CheckCircle className="h-3 w-3 mr-1"/>}
                                      {skill}
                                    </span>
                                );
