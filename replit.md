...
- **PDF page-break + Professional template WYSIWYG fixed (Apr 28 2026)**: Two related downloaded-PDF bugs reported. (1) Big blank space at the bottom of page 1 with everything after Summary jumping to page 2 — caused by the blanket rule `section { page-break-inside: avoid }` in `services/getCVHtml.ts`, which forced any too-tall section (Experience) to be pushed entirely to the next page. Replaced with finer-grained rules: `h2,h3,h4 { break-after: avoid }` keeps headings glued to following content, `li { break-inside: avoid }` keeps bullets atomic, and a new opt-in `[data-pdf-keep] { break-inside: avoid; page-break-inside: avoid; }` lets templates mark individual entries (one job, one degree) to stay together while letting the surrounding section flow. (2) `TemplateProfessional.tsx` preview did not match its downloaded PDF — root cause was responsive Tailwind classes that diverged between the small editor preview viewport and the 794×1123 PDF viewport: `p-8 sm:p-12` (different padding under sm breakpoint) and `hidden sm:inline` separators in the contact line. Switched to plain `p-12` and always-visible separators so the preview is faithful at any size. Added `data-pdf-keep="true"` to header, each experience entry, education entry, project entry, and publication entry in the Professional template.
