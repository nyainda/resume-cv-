...
- **AI CV Toolkit**:
    ...
    - **Quality Polish (shared)**: A single `runQualityPolishPasses` helper in `geminiService.ts` runs the humanizer + bullet-count enforcer + banned-phrase filter + experience sort + deterministic purify + pronoun fix. Used by `generateCV` (full path), `improveCV` (Auto Optimize), and `optimizeCVForJob` so all three flows produce CVs at parity. Source-CV bullet counts are preserved on improve/optimize so structure never silently changes.
