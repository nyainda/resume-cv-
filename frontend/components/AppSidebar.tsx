...
const NavLink: React.FC<NavLinkProps> = ({ item, isActive, isGated, darkMode: dark, onClick }) => (
  <button
    onClick={onClick}
    title={item.label}
    className={`w-full flex items-center justify-start md:justify-center xl:justify-start gap-2.5 pl-4 md:pl-0 md:pr-0 xl:pl-4 xl:pr-2 py-[7px] md:py-2.5 xl:py-[7px] text-[11.5px] font-semibold transition-all duration-150 text-left ${
  ...
  >
    ...
    <span className="flex-1 leading-tight md:hidden xl:block">{item.label}</span>
    {isGated && (
      <span
        className="text-[7px] font-black px-1 py-0.5 rounded mr-1 flex-shrink-0 md:hidden xl:inline-block"
...
const SidebarInner: React.FC<SidebarInnerProps> = ({
  ...
  const tierLabel = isPremium ? '★ Premium' : isByok ? '⚡ BYOK' : 'Free plan';
  const tierColor = isPremium ? GOLD : isByok ? '#7C3AED' : tw.userPlan(false, dark);
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <button
        ...
        className="w-full flex items-center justify-center md:justify-center xl:justify-start gap-2.5 px-4 md:px-0 xl:px-4 pt-5 pb-4 flex-shrink-0 hover:opacity-90 transition-opacity"
      >
        <img
          src="/logo.svg"
          alt="ProCV"
          ...
          draggable={false}
        />
        <div className="text-left leading-none md:hidden xl:block">
          <div
      ...
      {!isPremium && (
        <div
          className="hidden xl:block mx-2 mb-2 rounded-xl p-3 flex-shrink-0"
      ...
      >
        ...
        {/* Quick-action icon strip — md/lg icon-only mode: Notifications + Settings + dark mode toggle */}
        <div className="hidden md:flex xl:hidden flex-col items-center gap-0.5 px-2 pb-2">
          <button
            onClick={() => { onOpenNotifications?.(); onAfterNavClick?.(); }}
            title="Tips &amp; Activity"
            className={`w-full flex items-center justify-center py-2 rounded-lg transition-colors ${tw.iconBtn(dark)}`}
          >
...
const AppSidebar: React.FC<AppSidebarProps> = (props) => {
  ...
  const tierColor = isPremium ? GOLD : isByok ? '#7C3AED' : (dark ? 'rgba(255,255,255,0.45)' : '#6B7280');
  const tierBg = isPremium ? `${GOLD}25` : isByok ? 'rgba(124,58,237,0.15)' : (dark ? 'rgba(255,255,255,0.08)' : 'rgba(0,0,0,0.06)');
  const userInitial = (
