/**
 * SettingsPage — Full-page settings hub matching the dark-navy/gold design.
 *
 * Layout: Left nav (220px) | Main content (scrollable) | Right account panel (280px)
 * Storage references: CF D1 / IndexedDB / localStorage only — no Google Drive.
 */
...
const NAV_SECTIONS: NavSectionDef[] = [
  ...
  {
    id: 'profile-sharing', label: 'Profile & Sharing',
  ...
  {
    id: 'storage-backup', label: 'Storage & Backup',
  ...
  return (
    <div className="flex items-center justify-between mb-5">
      <div>
        ...
        {subtitle && <p style={{ color: 'rgba(255,255,255,0.45)', fontSize: 12, marginTop: 2 }}>{subtitle}</p>}
      </div>
    </div>
  );
}
...
function Card({ children, className = '' }: { children: React.ReactNode; className?: string }) {
  return (
    <div style={{ background: DARK_CARD, border: `1px solid ${DARK_BORDER}`, borderRadius: 12, padding: '16px 18px', marginBottom: 12 }} className={className}>
      {children}
    </div>
  );
}

// ── Subcomponent: Provider card (General overview) ────────────────────────────
function ProviderOverviewCard({
  ...
  return (
    <div style={{
      background: isActive ? 'rgba(27,43,75,0.6)' : 'rgba(255,255,255,0.03)',
      border: `1px solid ${isActive ? `${GOLD}40` : DARK_BORDER}`,
      borderRadius: 10,
      padding: '14px 16px',
      display: 'flex',
      alignItems: 'center',
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <p style={{ color: 'rgba(255,255,255,0.45)', fontSize: 11, margin: 0, lineHeight: 1.4 }}>{model}</p>
        <p style={{ color: 'rgba(255,255,255,0.3)', fontSize: 10, margin: '3px 0 0', lineHeight: 1.3 }}>{description}</p>
      </div>
      <button
        onClick={onConfigure}
        style={{
...
function StatTile({ icon, label, value, sub, status }: { icon: string; label: string; value: string; sub?: string; status?: 'ok' | 'warn' | 'info' }) {
  ...
  return (
    <div style={{ background: 'rgba(255,255,255,0.04)', border: `1px solid ${DARK_BORDER}`, borderRadius: 10, padding: '12px 14px', flex: 1, minWidth: 0 }}>
      <div style={{ fontSize: 18, marginBottom: 6 }}>{icon}</div>
      <div style={{ color: '#fff', fontWeight: 700, fontSize: 18 }}>{value}</div>
      <div style={{ color: 'rgba(255,255,255,0.5)', fontSize: 10, marginTop: 2 }}>{label}</div>
  ...
  return (
    <button
      ...
      style={{
        ...
        border: `1px solid ${isDanger ? 'rgba(239,68,68,0.2)' : DARK_BORDER}`,
        borderRadius: 10, padding: '12px 14px', cursor: 'pointer', transition: 'all 0.15s',
      }}
      onMouseEnter={e => { e.currentTarget.style.background = isDanger ? 'rgba(239,68,68,0.1)' : 'rgba(255,255,255,0.07)'; }}
      onMouseLeave={e => { e.currentTarget.style.background = isDanger ? 'rgba(239,68,68,0.06)' : 'rgba(255,255,255,0.04)'; }}
    >
      <div style={{ fontSize: 20, marginBottom: 6 }}>{icon}</div>
      <div style={{ color: isDanger ? '#f87171' : '#fff', fontWeight: 700, fontSize: 12 }}>{label}</div>
      <div style={{ color: 'rgba(255,255,255,0.35)', fontSize: 10, marginTop: 2 }}>{sub}</div>
    </button>
  );
}

// ── BYOK Provider card (full config) ─────────────────────────────────────────
...
function ByokCard({ id, icon, label, badge, badgeStyle, desc, keyValue, keyPlaceholder, keyLink, keyLinkLabel, borderGold, onKeyChange, modelValue, modelOptions, onModelChange, showCaching, selectedProvider, onSelect, providerTest, onTest }: ByokCardProps) {
  ...
  return (
    <div
      onClick={() => onSelect(id)}
      style={{
        background: active ? 'rgba(27,43,75,0.7)' : 'rgba(255,255,255,0.03)',
        border: `1.5px solid ${active ? GOLD + '50' : DARK_BORDER}`,
    ...
    >
      ...
      <p style={{ color: 'rgba(255,255,255,0.4)', fontSize: 11, margin: '0 0 6px', lineHeight: 1.4 }}>{desc}</p>
      {!active && (
        <span style={{ fontSize: 10, fontWeight: 600, color: hasKey ? '#22c55e' : 'rgba(255,255,255,0.3)' }}>
          {hasKey ? '● Key configured' : '○ No key yet'}
        </span>
      )}
      {active && (
        <div onClick={e => e.stopPropagation()} style={{ marginTop: 10, paddingTop: 10, borderTop: `1px solid ${DARK_BORDER}`, display: 'flex', flexDirection: 'column', gap: 8 }}>
          ...
          <input
            ...
            style={{
              background: 'rgba(255,255,255,0.06)', border: `1px solid ${DARK_BORDER}`,
          ...
          >
            {modelOptions.map(m => <option key={m.id} value={m.id} style={{ background: '#1a2740' }}>{m.label}</option>)}
          </select>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <button
              ...
              style={{
                ...
                background: 'rgba(255,255,255,0.1)', border: `1px solid ${DARK_BORDER}`,
                color: '#fff', cursor: hasKey ? 'pointer' : 'not-allowed', opacity: !hasKey ? 0.5 : 1,
              }}
            >
...
const SettingsPage: React.FC<SettingsPageProps> = ({
  ...
  const tierBadge = isPremium ? { label: 'Premium', bg: `${GOLD}20`, color: GOLD } :
    ...
    { label: 'Free', bg: 'rgba(255,255,255,0.08)', color: 'rgba(255,255,255,0.5)' };
  ...
  const renderGeneral = () => (
    <div>
      <SectionHeader
        title="AI Providers"
        subtitle="Choose and manage your AI providers and API keys."
        badge="How it works"
      />
      ...
      <Card>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
          <div>
            <p style={{ color: '#fff', fontWeight: 700, fontSize: 14, margin: 0 }}>API Key Vault</p>
            <p style={{ color: 'rgba(255,255,255,0.4)', fontSize: 11, margin: '2px 0 0' }}>Your keys are encrypted and never stored in plaintext.</p>
          </div>
          <button
            onClick={() => setActiveSection('ai-providers')}
            style={{ padding: '6px 14px', borderRadius: 8, fontSize: 11, fontWeight: 700, background: 'rgba(255,255,255,0.08)', border: `1px solid ${DARK_BORDER}`, color: '#fff', cursor: 'pointer' }}
          >
            Update Key
          </button>
        </div>
        ...
        {/* Masked key display */}
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, background: 'rgba(0,0,0,0.2)', borderRadius: 8, padding: '8px 12px', marginBottom: 12 }}>
          ...
          <span style={{ color: 'rgba(255,255,255,0.25)', fontSize: 13, fontFamily: 'monospace', flex: 1, letterSpacing: 4 }}>••••••••••••••••••••••••••••</span>
          <button style={{ fontSize: 10, color: 'rgba(255,255,255,0.3)', background: 'none', border: 'none', cursor: 'pointer', padding: 0 }}>
            👁
          </button>
        </div>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
          {[
            ...
            <div key={b.label} style={{ display: 'flex', alignItems: 'center', gap: 6, background: 'rgba(255,255,255,0.04)', border: `1px solid ${DARK_BORDER}`, borderRadius: 8, padding: '6px 10px' }}>
              <span style={{ fontSize: 14 }}>{b.icon}</span>
              <div>
                <div style={{ color: '#fff', fontSize: 10, fontWeight: 700 }}>{b.label}</div>
                <div style={{ color: 'rgba(255,255,255,0.35)', fontSize: 9 }}>{b.sub}</div>
              </div>
            </div>
          ))}
        </div>
      ...
      <Card>
        <p style={{ color: '#fff', fontWeight: 700, fontSize: 14, marginBottom: 4 }}>Security &amp; Usage Overview</p>
        <p style={{ color: 'rgba(255,255,255,0.4)', fontSize: 11, marginBottom: 12 }}>Real-time status of your account and security.</p>
        ...
        {tokenUsage.callCount > 0 && (
          <button
            ...
            style={{ marginTop: 10, fontSize: 10, color: 'rgba(255,255,255,0.3)', background: 'none', border: 'none', cursor: 'pointer', textDecoration: 'underline', padding: 0 }}
          >
      ...
      <Card>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <div>
              <p style={{ color: '#fff', fontWeight: 700, fontSize: 13, margin: 0 }}>Onboarding &amp; First Steps</p>
              <p style={{ color: 'rgba(255,255,255,0.4)', fontSize: 11, margin: '2px 0 0' }}>New to ProCV or want a refresher? Re-open the welcome wizard to get the most out of ProCV.</p>
            </div>
          </div>
          <button
            onClick={onOpenOnboarding}
            style={{ flexShrink: 0, padding: '7px 16px', borderRadius: 8, fontSize: 12, fontWeight: 700, background: GOLD, color: NAVY, cursor: 'pointer', border: 'none' }}
          >
            Replay Onboarding →
          </button>
        </div>
      ...
      <div style={{ marginTop: 16 }}>
        <p style={{ color: 'rgba(255,255,255,0.5)', fontSize: 11, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 10 }}>Quick Actions</p>
        <p style={{ color: 'rgba(255,255,255,0.3)', fontSize: 11, marginBottom: 12 }}>Shortcuts to common actions.</p>
  ...
  const renderAiProviders = () => (
    <div>
      ...
      {isFree && (
        <Card>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 }}>
            ...
            <div>
              <p style={{ color: '#fff', fontWeight: 700, fontSize: 13, margin: 0 }}>Free Tier — Workers AI</p>
              <p style={{ color: 'rgba(255,255,255,0.4)', fontSize: 11, margin: '2px 0 0' }}>Powered by Cloudflare Workers AI automatically. No key needed.</p>
            </div>
          </div>
          <p style={{ color: 'rgba(255,255,255,0.5)', fontSize: 12, lineHeight: 1.6, margin: '0 0 12px' }}>
      ...
      {/* BYOK / Premium key configuration */}
      {(isByok || isPremium) && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 16 }}>
          <ByokCard
            id="claude" icon="🧠" label="Claude (Anthropic)"
            badge={isPremium ? 'Optional' : 'Your key'} badgeStyle={{ background: 'rgba(139,92,246,0.15)', color: '#a78bfa' }}
            desc="Advanced reasoning, 200K context, prompt caching for 90% cost reduction on repeats."
            ...
            borderGold={selectedProvider === 'claude'} onKeyChange={setClaudeKey}
            modelValue={claudeModelState} modelOptions={CLAUDE_MODEL_OPTIONS} onModelChange={setClaudeModelState}
            showCaching selectedProvider={selectedProvider} onSelect={setSelectedProviderState}
            providerTest={selectedProvider === 'claude' ? providerTest : { status: 'idle' }} onTest={runProviderTest}
          />
          <ByokCard
            id="gemini" icon="✨" label="Gemini (Google)"
            badge={isPremium ? 'Optional' : 'Your key'} badgeStyle={{ background: 'rgba(59,130,246,0.15)', color: '#60a5fa' }}
            desc="Gemini 2.5 Flash via secure proxy. 1M context. Also enables PDF/image CV upload."
            keyValue={geminiKey} keyPlaceholder="AIzaSy..."
            keyLink="https://aistudio.google.com/app/apikey" keyLinkLabel="Get Gemini API key →"
            borderGold={selectedProvider === 'gemini'} onKeyChange={setGeminiKey}
            ...
            providerTest={selectedProvider === 'gemini' ? providerTest : { status: 'idle' }} onTest={runProviderTest}
          />
          <ByokCard
            id="groq" icon="⚡" label="Groq"
            badge={isPremium ? 'Optional' : 'Your key'} badgeStyle={{ background: 'rgba(249,115,22,0.15)', color: '#fb923c' }}
            desc="Ultra-fast inference — Llama 3.3 70B, Kimi K2, DeepSeek R1. Free tier generous."
            keyValue={groqKey} keyPlaceholder="gsk_..."
            keyLink="https://console.groq.com/keys" keyLinkLabel="Get Groq API key →"
            borderGold={selectedProvider === 'groq'} onKeyChange={setGroqKey}
            modelValue={groqModelState} modelOptions={GROQ_MODEL_OPTIONS} onModelChange={setGroqModelState}
            selectedProvider={selectedProvider} onSelect={setSelectedProviderState}
            providerTest={selectedProvider === 'groq' ? providerTest : { status: 'idle' }} onTest={runProviderTest}
          />
        </div>
      )}
  ...
  const renderSecurity = () => (
    <div>
      <SectionHeader title="Security" subtitle="How your data and keys are protected." />
      <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', marginBottom: 12 }}>
        {[
          { icon: '🔐', title: 'AES-256-GCM', desc: 'All sensitive data is encrypted with AES-256-GCM using your browser\'s Web Crypto API before any storage.' },
          { icon: '🗄️', title: 'IndexedDB Only', desc: 'Encrypted keys live in your browser\'s private IndexedDB. Never in localStorage, never in plaintext.' },
          { icon: '🚫', title: 'Zero-Knowledge', desc: 'Plaintext keys exist only in memory during your session. They are never written to disk or sent to our servers.' },
          { icon: '☁️', title: 'Cloudflare Proxy', desc: 'AI calls are routed through our Cloudflare Worker over HTTPS. Your API key never appears in browser network logs.' },
        ].map(item => (
          <div key={item.title} style={{ flex: '1 1 200px', background: 'rgba(255,255,255,0.04)', border: `1px solid ${DARK_BORDER}`, borderRadius: 10, padding: '14px 16px' }}>
            <div style={{ fontSize: 22, marginBottom: 8 }}>{item.icon}</div>
            <div style={{ color: '#fff', fontWeight: 700, fontSize: 13, marginBottom: 4 }}>{item.title}</div>
            <p style={{ color: 'rgba(255,255,255,0.4)', fontSize: 11, margin: 0, lineHeight: 1.5 }}>{item.desc}</p>
          </div>
        ))}
      </div>

      <Card>
        <p style={{ color: '#fff', fontWeight: 700, fontSize: 14, marginBottom: 10 }}>Session Usage</p>
        ...
        <p style={{ color: 'rgba(255,255,255,0.3)', fontSize: 10 }}>~1 token ≈ 4 characters. Resets on page reload.</p>
        {tokenUsage.callCount > 0 && (
    ...
    <div>
      ...
      <Card>
        <div style={{ display: 'flex', gap: 12, alignItems: 'flex-start' }}>
          ...
          <div>
            <p style={{ color: '#fff', fontWeight: 700, fontSize: 14, margin: '0 0 6px' }}>Share Your CV / Profile</p>
            ...
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
              {[
                ...
                <div key={f.label} style={{ display: 'flex', alignItems: 'center', gap: 6, background: 'rgba(255,255,255,0.04)', border: `1px solid ${DARK_BORDER}`, borderRadius: 8, padding: '6px 10px' }}>
                  <span style={{ fontSize: 14 }}>{f.icon}</span>
                  <div>
                    <div style={{ color: '#fff', fontSize: 10, fontWeight: 700 }}>{f.label}</div>
                    <div style={{ color: 'rgba(255,255,255,0.35)', fontSize: 9 }}>{f.sub}</div>
                  </div>
                </div>
              ))}
            </div>
      ...
      <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
        ...
        <div style={{ flex: 1, minWidth: 220, background: 'rgba(255,255,255,0.03)', border: `1px solid ${DARK_BORDER}`, borderRadius: 12, padding: 16 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
            <span style={{ fontSize: 20 }}>🔗</span>
            <div>
              <span style={{ color: '#fff', fontWeight: 700, fontSize: 13 }}>Temporary Share Link</span>
          ...
          <p style={{ color: 'rgba(255,255,255,0.4)', fontSize: 11, lineHeight: 1.5, margin: '0 0 12px' }}>
            Creates a short, expiring link to share your CV instantly. Expires in 30 days. Rate limited to 10 links/hour.
          </p>
          <div style={{ display: 'flex', gap: 8 }}>
            <div style={{ flex: 1, background: 'rgba(0,0,0,0.3)', borderRadius: 8, padding: '7px 10px', fontFamily: 'monospace', fontSize: 11, color: 'rgba(255,255,255,0.4)' }}>
              https://procv.app/#s=…
            </div>
        ...
        <div style={{ flex: 1, minWidth: 220, background: 'rgba(255,255,255,0.03)', border: `1px solid ${DARK_BORDER}`, borderRadius: 12, padding: 16 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
            <span style={{ fontSize: 20 }}>🌐</span>
            <div>
              <span style={{ color: '#fff', fontWeight: 700, fontSize: 13 }}>Public Profile</span>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <span style={{ color: 'rgba(255,255,255,0.3)', fontSize: 11 }}>procv.app/p/</span>
            <input
              placeholder="your-slug"
              style={{ flex: 1, background: 'rgba(0,0,0,0.3)', border: `1px solid ${DARK_BORDER}`, borderRadius: 7, padding: '6px 10px', color: '#fff', fontSize: 11, outline: 'none' }}
            />
          </div>
        </div>
      </div>
    ...
    <div>
      ...
      <div style={{ display: 'flex', gap: 10, marginBottom: 12, flexWrap: 'wrap' }}>
        {[
          ...
          <div key={item.title} style={{ flex: '1 1 180px', background: 'rgba(255,255,255,0.04)', border: `1px solid ${DARK_BORDER}`, borderRadius: 12, padding: '14px 16px' }}>
            ...
            <p style={{ color: '#fff', fontWeight: 700, fontSize: 13, margin: '0 0 5px' }}>{item.title}</p>
            <p style={{ color: 'rgba(255,255,255,0.4)', fontSize: 11, margin: '0 0 8px', lineHeight: 1.4 }}>{item.desc}</p>
            <p style={{ color: GOLD, fontSize: 10, fontWeight: 600, margin: 0 }}>{item.detail}</p>
          </div>
        ))}
      </div>

      {/* Auto-save & sync settings */}
      <Card>
        <p style={{ color: '#fff', fontWeight: 700, fontSize: 14, marginBottom: 12 }}>Sync Settings</p>
        {[
          ...
          <div key={row.label} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 0', borderBottom: `1px solid ${DARK_BORDER}` }}>
            <div>
              <span style={{ color: '#fff', fontSize: 13, fontWeight: 600 }}>{row.label}</span>
              <p style={{ color: 'rgba(255,255,255,0.35)', fontSize: 10, margin: '2px 0 0' }}>{row.desc}</p>
            </div>
      ...
      <Card>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <div>
              <p style={{ color: '#fff', fontWeight: 700, fontSize: 13, margin: 0 }}>Full Storage Map</p>
              <p style={{ color: 'rgba(255,255,255,0.4)', fontSize: 11, margin: '2px 0 0' }}>See every localStorage key, IndexedDB entry, and CF D1 table — with sizes and sync destinations.</p>
            </div>
          </div>
          <button
            onClick={() => {}}
            style={{ padding: '7px 14px', borderRadius: 8, fontSize: 11, fontWeight: 700, background: 'rgba(255,255,255,0.08)', border: `1px solid ${DARK_BORDER}`, color: '#fff', cursor: 'pointer', whiteSpace: 'nowrap' }}
          >
            View Storage Map →
          </button>
        </div>
  ...
  const renderBilling = () => (
    <div>
      ...
      <Card>
        <p style={{ color: '#fff', fontWeight: 700, fontSize: 14, marginBottom: 14 }}>Choose Your Power</p>
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
            <thead>
              <tr>
                <td style={{ padding: '6px 12px', color: 'rgba(255,255,255,0.4)', fontSize: 11 }}></td>
                {[
            ...
            <tbody>
              {[
                ...
                <tr key={row.feature} style={{ background: i % 2 === 0 ? 'rgba(255,255,255,0.02)' : 'transparent' }}>
                  <td style={{ padding: '7px 12px', color: 'rgba(255,255,255,0.6)', fontSize: 11 }}>{row.feature}</td>
                  {[row.free, row.byok, row.premium].map((val, ci) => (
                    <td key={ci} style={{
                      padding: '7px 16px', textAlign: 'center', fontSize: 11, fontWeight: 600,
                      color: val === '✗' ? 'rgba(255,255,255,0.2)' : ci === 2 ? NAVY : val.includes('✓') ? '#22c55e' : '#fff',
  ...
  const renderAppearance = () => (
    <div>
      ...
      <Card>
        <p style={{ color: '#fff', fontWeight: 700, fontSize: 13, marginBottom: 14 }}>Theme</p>
          ...
          {[
            ...
            <button
              ...
              style={{
                ...
                background: darkMode === opt.value ? `${GOLD}20` : 'rgba(255,255,255,0.04)',
                border: `2px solid ${darkMode === opt.value ? GOLD : DARK_BORDER}`,
                display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8,
              }}
            >
              <span style={{ fontSize: 24 }}>{opt.icon}</span>
              <span style={{ color: darkMode === opt.value ? GOLD : 'rgba(255,255,255,0.6)', fontSize: 12, fontWeight: 700 }}>{opt.label}</span>
            </button>
          ))}
        </div>
      </Card>
  ...
  const renderAdvanced = () => (
    <div>
      ...
      <Card>
        <p style={{ color: '#fff', fontWeight: 700, fontSize: 13, marginBottom: 10 }}>Export Account Data</p>
        <p style={{ color: 'rgba(255,255,255,0.4)', fontSize: 12, lineHeight: 1.5, marginBottom: 12 }}>
        ...
        <button
          ...
          style={{ padding: '7px 16px', borderRadius: 8, fontSize: 12, fontWeight: 700, background: 'rgba(255,255,255,0.08)', border: `1px solid ${DARK_BORDER}`, color: '#fff', cursor: 'pointer' }}
        >
          📤 Export JSON
        </button>
      </Card>
      ...
      <Card>
        <p style={{ color: '#fff', fontWeight: 700, fontSize: 13, marginBottom: 10 }}>Clear Local Cache</p>
        <p style={{ color: 'rgba(255,255,255,0.4)', fontSize: 12, lineHeight: 1.5, marginBottom: 12 }}>
        ...
        <button
          ...
          style={{ padding: '7px 16px', borderRadius: 8, fontSize: 12, fontWeight: 700, background: 'rgba(255,255,255,0.08)', border: `1px solid ${DARK_BORDER}`, color: '#fff', cursor: 'pointer', opacity: clearingData ? 0.6 : 1 }}
        >
      ...
      {/* Sign Out */}
      <Card>
        <p style={{ color: '#fff', fontWeight: 700, fontSize: 13, marginBottom: 6 }}>Sign Out</p>
        <p style={{ color: 'rgba(255,255,255,0.4)', fontSize: 12, margin: '0 0 12px' }}>End your current session. Your data remains saved.</p>
        ...
        {deletingStep === 'idle' ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            ...
            <input
              ...
              placeholder="delete"
              style={{ background: 'rgba(0,0,0,0.3)', border: '1px solid rgba(239,68,68,0.4)', borderRadius: 8, padding: '7px 12px', color: '#fff', fontSize: 12, outline: 'none', width: '100%', boxSizing: 'border-box' }}
            />
            <div style={{ display: 'flex', gap: 8 }}>
              <button
              ...
              <button onClick={() => { setDeletingStep('idle'); setConfirmText(''); }} style={{ padding: '7px 16px', borderRadius: 8, fontSize: 12, fontWeight: 600, background: 'rgba(255,255,255,0.06)', border: `1px solid ${DARK_BORDER}`, color: 'rgba(255,255,255,0.6)', cursor: 'pointer' }}>
                Cancel
              </button>
            </div>
          </div>
  ...
  return (
    <div style={{
      minHeight: '100vh',
      background: 'linear-gradient(135deg, #0a1628 0%, #0d1d35 50%, #0a1220 100%)',
        ...
        <div style={{ width: 270, flexShrink: 0, borderLeft: `1px solid ${DARK_BORDER}`, padding: '20px 16px', overflowY: 'auto' }}>
          ...
          <div style={{ borderTop: `1px solid ${DARK_BORDER}`, paddingTop: 14, marginBottom: 14 }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
              <p style={{ fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', color: 'rgba(255,255,255,0.3)', margin: 0 }}>Your Profile Slots</p>
              <span style={{ fontSize: 10, color: 'rgba(255,255,255,0.3)' }}>{profiles.length} of {isPremium ? 5 : isByok ? 3 : 1} used</span>
            </div>
              ...
              {profiles.slice(0, 5).map(slot => {
                ...
                return (
                  <div
                    ...
                    style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '6px 8px', borderRadius: 7, background: isActive ? `${GOLD}12` : 'rgba(255,255,255,0.03)', border: `1px solid ${isActive ? GOLD + '30' : DARK_BORDER}`, cursor: 'pointer', transition: 'all 0.15s' }}
                  >
                    <div style={{ width: 8, height: 8, borderRadius: '50%', background: slot.color === 'amber' ? '#f59e0b' : slot.color === 'emerald' ? '#10b981' : slot.color === 'violet' ? '#7c3aed' : GOLD, flexShrink: 0 }} />
                    <span style={{ color: isActive ? '#fff' : 'rgba(255,255,255,0.65)', fontSize: 11, fontWeight: isActive ? 700 : 500, flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                      {slot.name}
                    </span>
                    {isPrimary && <span style={{ fontSize: 8, fontWeight: 700, padding: '1px 5px', borderRadius: 10, background: `${GOLD}20`, color: GOLD }}>Primary</span>}
                    {!isPrimary && isActive && <span style={{ fontSize: 8, fontWeight: 700, padding: '1px 5px', borderRadius: 10, background: 'rgba(34,197,94,0.15)', color: '#22c55e' }}>Active</span>}
                  </div>
                );
              })}
            </div>
          ...
          <div style={{ borderTop: `1px solid ${DARK_BORDER}`, paddingTop: 14, marginBottom: 14 }}>
            <p style={{ fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', color: 'rgba(255,255,255,0.3)', margin: '0 0 8px' }}>Storage &amp; Backup</p>
            <p style={{ fontSize: 10, color: 'rgba(255,255,255,0.3)', margin: '0 0 8px' }}>Everything is synced and safe.</p>
            {[
