...
function ScoreGauge({ target, active }: { target: number; active: boolean }) {
  ...
  const startFrac = (31 - 0) / 100;
  const endFrac = (target - 0) / 100;
...
function TemplateCard({ tmpl, pos, delay, active }: { tmpl: typeof templates[0]; pos: typeof templatePositions[0]; delay: number; active: boolean }) {
  return (
    <motion.div
      ...
      animate={active ? { opacity: 1, scale: pos.scale, rotate: pos.rotate, x: pos.x, y: pos.y } : { opacity: 0 }}
      transition={{ duration: 0.8, delay, type: 'spring', stiffness: 180, damping: 18 }}
    >
      <div className="p-[0.8vw]">
        ...
        {[1, 0.7, 0.85, 0.6, 0.9, 0.5].map((w, i) => (
          <div key={i} className="h-[0.18vw] rounded-full mb-[0.4vw]" style={{ background: 'rgba(255,255,255,0.15)', width: `${w * 100}%` }} />
        ))}
        ...
        {[0.8, 0.6, 0.75].map((w, i) => (
          <div key={i} className="h-[0.18vw] rounded-full mb-[0.4vw]" style={{ background: 'rgba(255,255,255,0.12)', width: `${w * 100}%` }} />
        ))}
      </div>
    </motion.div>
  );
    ...
    >
      ...
      <div className="relative flex items-center justify-center" style={{ height: '50vh' }}>
        {templates.map((tmpl, i) => (
          ...
          <TemplateCard tmpl={tmpl} pos={templatePositions[i]} delay={i * 0.1} active={phase >= 3} key={String(i)} />
        ))}
      ...
      <motion.p
        ...
        style={{ color: '#F8F7F4', fontFamily: 'Playfair Display, serif', position: 'relative', zIndex: 10 }}
        initial={{ opacity: 0 }}
