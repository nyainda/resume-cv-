        ...
        <div id="cv-preview-swe-elite" className="bg-white flex font-sans text-sm" style={{ minHeight: '297mm', fontFamily: "'Inter', system-ui, sans-serif" }}>
            ...
            <aside className="w-[220px] flex-shrink-0 bg-[#0f172a] text-white flex flex-col py-8 px-6 space-y-7" style={{ minHeight: '297mm' }}>
                ...
                <div className="text-center">
                    <div className="w-16 h-16 rounded-2xl flex items-center justify-center text-2xl font-black text-white mx-auto mb-3 ring-4 bg-slate-700">
                        {initials}
                    </div>
                    <h1 className="text-lg font-black leading-tight text-white">{personalInfo.name}</h1>
                    {cvData.experience[0] && (
                        <p className="text-xs font-semibold mt-1" style={{ color: '#94a3b8' }}>{cvData.experience[0].jobTitle}</p>
                    )}
                </div>
                            ...
                            <div key={cat}>
                                ...
                                <div className="flex flex-wrap gap-1">
                                    {skills.slice(0, 8).map(skill => (
                                        <span key={skill} className="text-[10px] px-1.5 py-0.5 rounded font-mono font-semibold leading-tight text-slate-300" style={{ backgroundColor: 'rgba(148,163,184,0.1)', border: '1px solid rgba(148,163,184,0.2)' }}>
                                            {skill}
                                        </span>
                                    ))}
                                    {skills.length > 8 && (
                ...
                {cvData.education.length > 0 && (
                    <div className="space-y-2">
                        ...
                        {cvData.education.length > 2 && (
                            <p className="text-[10px] mt-1 pt-1 border-t border-slate-700 text-slate-500">
                    ...
                    <div className="space-y-2">
                        ...
                        {cvData.languages.map((lang, i) => (
                            <div key={i} className="flex justify-between items-center">
                                ...
                                <span className="text-[10.5px] font-semibold text-slate-500">{lang.proficiency}</span>
                            </div>
                        ))}
                    </div>
                )}
                                ...
                                <div key={i} className="relative pl-4 border-l-2 transition-colors" style={{ borderColor: accent + '33' }}>
                                    <div className="flex items-start justify-between gap-2 flex-wrap">
                                        <div>
                                            ...
                                            />
                                            <p className="font-semibold text-xs mt-0.5" style={{ color: INK_SUBLINE }}
                                    ...
                                    <ul className="mt-2 space-y-1">
                                        {job.responsibilities.map((resp, j) => (
                                            <li key={j} className="flex items-start gap-2 text-xs text-slate-600 leading-relaxed">
                                                <span className="mt-0.5 font-black flex-shrink-0" style={{ color: INK_DOT }}>▸</span>
                                    ...
                                    <div className="flex items-start justify-between gap-2">
                                        ...
                                        {proj.link && (
                                            <a href={proj.link} target="_blank" rel="noopener noreferrer"
                                                className="text-[10.5px] font-bold hover:underline flex-shrink-0 flex items-center gap-0.5" style={{ color: INK_LINK }}>
