...
const TemplateCompact: React.FC<TemplateProps> = ({ cvData, personalInfo, isEditing, onDataChange, jobDescriptionForATS }) => {
  ...
  const MainSection: React.FC<{ title: string; children: React.ReactNode }> = ({ title, children }) => (
    <section className="mb-6">
      <h2 className="text-sm font-bold uppercase tracking-wider border-b border-slate-300 pb-1 mb-3" style={{ color: accent }}>{title}</h2>
      {children}
    </section>
  );
  ...
  const SidebarSection: React.FC<{ title: string; children: React.ReactNode }> = ({ title, children }) => (
    <section className="mb-6">
      <h2 className="text-sm font-bold uppercase tracking-wider pb-1 mb-2" style={{ color: accent }}>{title}</h2>
      {children}
    </section>
  );
    ...
    <div id="cv-preview-compact" className="bg-white p-8 text-slate-800 shadow-lg border font-['Inter'] text-[10pt] leading-snug">
      ...
      <div className="grid grid-cols-12 gap-x-8">
              ...
              {cvData.experience.map((job, index) => (
                <div key={index}>
                  ...
                  <ul className="list-disc list-outside ml-4 mt-1 space-y-1 text-slate-700">
                    {smartBullets(job.responsibilities, cvData.experience.length).map((resp, i) => <li key={i} dangerouslySetInnerHTML={{ __html: resp }} {...editableProps(['experience', index, 'responsibilities', i])} />)}
                  </ul>
                </div>
              ))}
            </div>
        ...
        <TemplateCustomSections
          ...
          renderHeader={title => <h2 className="text-sm font-bold uppercase tracking-wider border-b border-slate-300 pb-1 mb-3" style={{ color: accent }}>{title}</h2>}
