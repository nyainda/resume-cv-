...
const TemplateTokyoNight: React.FC<TemplateProps> = ({ cvData, personalInfo, isEditing, onDataChange, jobDescriptionForATS }) => {
    ...
    const SectionHeader = ({ title }: { title: string }) => (
        <div className="flex items-center gap-3 mb-3">
            <h2 className="text-[10px] font-black uppercase tracking-[0.2em] px-2 py-0.5 rounded border" style={{ color: accent, backgroundColor: accent + '1a', borderColor: accent + '33' }}>{title}</h2>
            <div className="flex-1 h-[1px]" style={{ background: `linear-gradient(to right, ${accent}33, transparent)` }}></div>
        </div>
    );
    ...
    return (
        <div id="cv-preview-tokyo-night" className="bg-[#1a1b26] p-7 text-slate-300 shadow-2xl border border-slate-800 font-mono flex flex-col min-h-[280mm]" style={{ fontFamily: "'JetBrains Mono', 'Fira Code', monospace" }}>
            <header className="mb-4 border-l-4 border-fuchsia-500 pl-5">
                <h1 className="text-2xl font-black tracking-tighter text-white mb-1 uppercase italic">
                    {personalInfo.name}
                </h1>
            ...
            <main className="grid grid-cols-12 gap-6 flex-1">
                ...
                <div className="col-span-8 flex flex-col gap-4">
                        ...
                        <div className="space-y-4">
                            {cvData.experience.map((job, index) => (
                                <div key={index} className="relative group pl-3 border-l border-fuchsia-500/30 hover:border-fuchsia-500 transition-colors">
                                    ...
                                    <p className="text-xs font-bold text-fuchsia-400 mb-1.5" {...editableProps(['experience', index, 'company'])}>{job.company}</p>
                    ...
                    <TemplateCustomSections
                      ...
                      titleClass="font-semibold text-xs text-white"
                      subtitleClass="text-[10px] opacity-60"
                      descClass="text-[10px] opacity-70 mt-0.5"
                      yearClass="text-[10px] opacity-50"
                    />
                </div>
                    ...
                    <section>
                        ...
                        <div className="flex flex-wrap gap-1.5">
                            {cvData.skills.slice(0, 14).map((skill, i) => (
                                <span key={i} className="text-[10px] font-black uppercase px-1.5 py-0.5 bg-slate-800 text-slate-400 border border-slate-700 hover:border-cyan-400/50 hover:text-white transition-all">
                                    {skill}
                                </span>
                            ))}
                        </div>
                            ...
                            <div className="space-y-2">
                                {cvData.projects.slice(0, 3).map((proj, i) => (
                                    <div key={i} className="border-l border-fuchsia-500/20 pl-3">
                                        <p className="text-[10px] font-black text-white uppercase leading-snug">{proj.name}</p>
                                        <p className="text-[10px] text-slate-500 leading-snug line-clamp-2 mt-0.5" dangerouslySetInnerHTML={{ __html: proj.description }} />
