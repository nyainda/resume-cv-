...
const TemplateSoftwareEngineer: React.FC<TemplateProps> = ({ cvData, personalInfo, isEditing, onDataChange, jobDescriptionForATS }) => {
  const accent = cvData.accentColor ?? '#1e293b';
  ...
  const Section: React.FC<{ title: string; children: React.ReactNode }> = ({ title, children }) => (
    <section className="mb-6">
      <h2 className="text-base font-bold uppercase tracking-wider border-b-2 border-slate-200 pb-1 mb-3" style={{ color: accent }}>{title}</h2>
      {children}
    </section>
  );
