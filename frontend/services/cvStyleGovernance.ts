...
const NOUN_SUBJECT_RX = /^(?:payment|cart|customer|conversion|retention|churn|attrition|error|failure|performance|throughput|latency|uptime|satisfaction|spend|headcount|load|response|availability|capacity|efficiency|accuracy)(?:\s+[a-z]+){0,2}\s+(?:rates?|times?|costs?|revenue|margins?|volumes?|scores?|ratios?|adoption|retention|churn|attrition|errors?|failures?|abandonment|performance|throughput|latency|uptime|satisfaction|spend|headcount|conversion|load|response|availability|capacity|efficiency|accuracy)\b/i;
