 ...
 */
const IMPACT_OPENER_RX = /^(built|created|designed|developed|launched|delivered|drove|generated|secured|won|saved|reduced|increased|improved|led|deployed|released|shipped|achieved|exceeded|surpassed|grew|expanded|scaled|negotiated|closed|raised|recovered|rebuilt|re-engineered|re-designed|overhauled|transformed|automated|migrated|established|founded|pioneered|introduced|rolled\s+out|implemented|executed|completed|finished|produced|published|authored|wrote|presented|trained|hired|managed|configured|integrated|architected|engineered|streamlined|optimised|optimized|analysed|analyzed|consolidated|standardised|standardized|restructured|revamped|accelerated|secured|enabled|facilitated|coordinated|directed|spearheaded|orchestrated|championed|drove|delivered|negotiated|onboarded|mentored|coached|recruited|retained|partnered|liaised|audited|reviewed|assessed|evaluated|diagnosed|resolved|troubleshot|debugged|refactored|modernised|modernized|containerised|containerized|deployed|provisioned|monitored|instrumented|documented|defined|formulated|crafted|shaped|identified|uncovered|piloted|trialled|validated|tested|certified)\b/i;

/**
 * Outcome words that, combined with an impact opener, confirm an achievement.
 */
const OUTCOME_WORDS_RX = /\b(result|outcome|impact|success|revenue|cost|time|efficiency|performance|productivity|quality|growth|reduction|saving|improvement|increase|decrease|gain|loss|ROI|profit|churn|retention|conversion|engagement|adoption|throughput|latency|uptime|downtime|coverage|capacity|scale|reach|audience|team|client|customer|user|deal|contract|award|certification|record|target|goal|deadline|budget|headcount|accuracy|reliability|compliance|security|stability|availability|resilience|velocity|cycle|lead\s+time|error\s+rate|defect|incident|risk|savings|waste|backlog|pipeline|deployment|release|feature|platform|system|infrastructure|process|workflow|framework|architecture|integration|migration|rollout|adoption|onboarding|satisfaction|score|rating|rank|position|market|share|base|portfolio|account|region|territory|country|site|plant|project|programme|initiative|rollout|launch)\b/i;
