...
// ── Extracted modules (split for readability — logic unchanged) ──────────────
import { POLISH_STAGE_EVENT, type PolishStageId, type PolishStagePayload, _dispatchPolishStage } from './gemini/polishStage';
import { _getBannedPhrasesForPrompt } from './gemini/bannedPhrasesPrompt';
import { shuffleArray } from './gemini/varianceHelpers';
import {
    CV_RULES_VERSION, cloneCVData, cvCacheKey, cvCacheGet, cvCacheSet,
} from './gemini/cvGenerationCache';
import { deleteCVCacheEntry, invalidateCVCache } from './cvCache';
import {
    _normalizeCurrencyInCV, detectCurrency, detectSeniority, detectScenario,
    detectDomainPivot, buildPivotBlock, buildScenarioBlock, detectMarket,
    detectGaps, buildModePromptBlock, buildMetricsCeiling,
} from './gemini/preGeneration';
import {
    loadRules,
    HUMANIZATION_RULES, HUMANIZATION_CHECKLIST,
    SYSTEM_INSTRUCTION_PROFESSIONAL, SYSTEM_INSTRUCTION_PARSER, SYSTEM_INSTRUCTION_HUMANIZER,
    CV_DATA_SCHEMA, _criticalRulesReminder, _humanizationInstructionHeader,
    _validatorSystem, _auditSystem,
} from './gemini/rulesState';
import {
    getGeminiClient, getClaudeApiKey, claudeMultimodalCall, retryGemini,
} from './gemini/multimodalClients';
import {
    compactProfile, smartTruncateJD, jdProfileSimilarity, slimPromptProfile,
} from './gemini/profileSerialize';

// Re-export extracted public API so existing imports from './geminiService' keep working
export {
    POLISH_STAGE_EVENT,
    type PolishStageId,
