...
function makeCV(overrides: Partial<CVData> = {}): CVData {
  return {
    summary: 'Experienced software engineer with 5 years delivering scalable backend systems.',
    ...
    education: [
      { degree: 'BSc Computer Science', school: 'University of Edinburgh', year: '2019' },
    ],
    ...overrides,
  };
}
...
describe('skills_cap', () => {
  it('passes when skills count is at or below 15', () => {
    const cv = makeCV({ skills: Array.from({ length: 15 }, (_, i) => `Skill${i}`) });
    const result = runValidationEngine(cv);
    expect(result.passed).toBe(true);
    expect(result.violations.filter(v => v.ruleId === 'skills_cap')).toHaveLength(0);
  });

  it('fires and auto-repairs when skills exceed 15', () => {
    const cv = makeCV({ skills: Array.from({ length: 20 }, (_, i) => `Skill${i}`) });
    const result = runValidationEngine(cv);
...
describe('skills_deduplication', () => {
  it('passes when no duplicates', () => {
    ...
    const result = runValidationEngine(cv);
    expect(result.violations.filter(v => v.ruleId === 'skills_deduplication')).toHaveLength(0);
  });

  it('removes case-insensitive duplicates', () => {
    const cv = makeCV({ skills: ['TypeScript', 'typescript', 'TYPESCRIPT', 'React'] });
    const result = runValidationEngine(cv);
    expect(result.cv.skills.length).toBeLessThan(4);
    const tsCount = result.cv.skills.filter(s => s.toLowerCase() === 'typescript').length;
...
describe('no_seeking_phrases', () => {
  it('passes when summary has no seeking phrases', () => {
    const cv = makeCV();
    const result = runValidationEngine(cv);
    expect(result.violations.filter(v => v.ruleId === 'no_seeking_phrases')).toHaveLength(0);
  });

  it('detects and repairs "seeking" phrase', () => {
    const cv = makeCV({ summary: 'I am currently seeking a senior engineering role at a growth-stage startup.' });
    const result = runValidationEngine(cv);
    expect(result.violations.find(v => v.ruleId === 'no_seeking_phrases')).toBeDefined();
    expect(result.cv.summary.toLowerCase()).not.toMatch(/seeking/);
    expect(result.repairApplied).toBe(true);
  });

  it('detects and repairs "looking for" phrase', () => {
    const cv = makeCV({ summary: 'A results-driven engineer looking for new challenges.' });
    const result = runValidationEngine(cv);
  ...
  it('detects "I am a" opener', () => {
    const cv = makeCV({ summary: 'I am a passionate software engineer with 5 years of experience.' });
    const result = runValidationEngine(cv);
    const v = result.violations.find(v => v.ruleId === 'no_seeking_phrases');
    expect(v).toBeDefined();
  });
  ...
  it('detects "my goal is" phrase', () => {
    const cv = makeCV({ summary: 'My goal is to leverage my skills in a dynamic team environment.' });
    const result = runValidationEngine(cv);
    const v = result.violations.find(v => v.ruleId === 'no_seeking_phrases');
    expect(v).toBeDefined();
  });
  ...
  it('does not mutate the input CVData', () => {
    const cv = makeCV({ summary: 'I am currently seeking a senior engineering role.' });
...
describe('no_first_person_summary', () => {
  it('passes for a third-person summary', () => {
    const cv = makeCV({ summary: 'Experienced product manager with 8 years in B2B SaaS.' });
    const result = runValidationEngine(cv);
    expect(result.violations.find(v => v.ruleId === 'no_first_person_summary')).toBeUndefined();
  });
  ...
  it('warns for "I am" opener (non-seeking form)', () => {
    ...
    const cv = makeCV({ summary: 'I am determined to build reliable, high-performance software systems.' });
    const result = runValidationEngine(cv);
    const v = result.violations.find(v => v.ruleId === 'no_first_person_summary');
    expect(v).toBeDefined();
  ...
  it('warns for "I\'ve" opener', () => {
    ...
    const result = runValidationEngine(cv);
    expect(result.violations.find(v => v.ruleId === 'no_first_person_summary')).toBeDefined();
  });
...
describe('empty_experience_bullets', () => {
  it('passes when all roles have bullets', () => {
    const cv = makeCV();
    const result = runValidationEngine(cv);
    expect(result.violations.filter(v => v.ruleId === 'empty_experience_bullets')).toHaveLength(0);
  });
  ...
  it('warns when a role has zero bullets', () => {
    ...
    const result = runValidationEngine(cv);
    const v = result.violations.find(v => v.ruleId === 'empty_experience_bullets');
    expect(v).toBeDefined();
    expect(v!.severity).toBe('warn');
  });
...
describe('hollow_bullets', () => {
  it('passes when all bullets are long enough', () => {
    const cv = makeCV();
    const result = runValidationEngine(cv);
    expect(result.violations.filter(v => v.ruleId === 'hollow_bullets')).toHaveLength(0);
  });
...
describe('overlong_bullets', () => {
  it('passes when all bullets are under 50 words', () => {
    const cv = makeCV();
    const result = runValidationEngine(cv);
    expect(result.violations.filter(v => v.ruleId === 'overlong_bullets')).toHaveLength(0);
  });
...
describe('duplicate_openers', () => {
  it('passes when all bullets open with different verbs', () => {
    const cv = makeCV();
    const result = runValidationEngine(cv);
    expect(result.violations.filter(v => v.ruleId === 'duplicate_openers')).toHaveLength(0);
  });
...
describe('excess_bullets', () => {
  it('passes when bullet count is within tolerance (targetBulletCount + 2)', () => {
    ...
    expect(result.violations.filter(v => v.ruleId === 'excess_bullets')).toHaveLength(0);
  });
  ...
  it('warns when bullet count exceeds target + 2', () => {
    const cv = makeCV({
      experience: [{
        ...
        startDate: '2020-01-01',
        endDate: 'Present',
        responsibilities: Array.from({ length: 10 }, (_, i) =>
          `Delivered project ${i + 1} under budget and on schedule with measurable outcome.`
        ),
      }],
    });
    const result = runValidationEngine(cv, { targetBulletCount: 5 });
    const v = result.violations.find(v => v.ruleId === 'excess_bullets');
    expect(v).toBeDefined();
    expect(v!.severity).toBe('warn');
  });
